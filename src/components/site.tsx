...
export function TrailerDialog({ videoId }: { videoId: string }) {
  return (
    <dialog id="trailer-dialog" data-video-id={videoId} className="fixed inset-0 z-[100] m-0 hidden h-dvh w-dvw max-w-none items-center justify-center border-0 bg-black/80 p-4 opacity-0 backdrop-blur-sm pointer-events-none transition-opacity duration-200" onCancel={(event) => { event.preventDefault(); closeTrailerDialog(); }} onKeyDown={(event) => { if (event.key === "Escape") { event.preventDefault(); closeTrailerDialog(); } }} onClick={(event) => { if (event.target === event.currentTarget) closeTrailerDialog(); }}>
      <div className="relative aspect-video overflow-hidden rounded-xl bg-black shadow-2xl" style={{ width: "min(100%, calc((100dvh - 2rem) * 16 / 9), 64rem)" }}>
        <iframe id="trailer-iframe" title="Dead or Alive 6 Last Round trailer" tabIndex={-1} className="size-full border-0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
